...
fun PassengerSuperApp() {
    ...
    val prefs = remember { ctx.getSharedPreferences("bayra_p_v210", Context.MODE_PRIVATE) }
    ...
    if (!isAuth) {
        ...
        ModalNavigationDrawer(
            ...
            gesturesEnabled = false, // 🔥 GESTURE LOCKDOWN: Swipe to open is disabled.
            drawerContent = {
                ModalDrawerSheet {
                    Column(Modifier.padding(20.dp)) {
                        Icon(Icons.Filled.AccountCircle, null, Modifier.size(64.dp), IMPERIAL_BLUE)
                        Text(pName, fontWeight = FontWeight.Bold, fontSize = 18.sp)
                    ...
                    Divider()
                    NavigationDrawerItem(label = { Text("Invite Friends") }, selected = false, onClick = { 
                        val intent = Intent(Intent.ACTION_SEND).apply { type = "text/plain"; putExtra(Intent.EXTRA_TEXT, "Join me on Bayra Travel Arba Minch! Download here: [Your Link]") }
                        ctx.startActivity(Intent.createChooser(intent, "Share Bayra"))
...
fun SettingsPage(isDarkMode: Boolean, onToggle: (Boolean) -> Unit) {
    ...
    Column(Modifier.fillMaxSize().padding(24.dp)) {
        ...
        Text("Support & Contact", fontWeight = FontWeight.Bold, color = Color.Gray)
...
fun AboutUsPage() {
    Column(Modifier.fillMaxSize().padding(24.dp).verticalScroll(rememberScrollState())) {
        ...
        Text("Policy & Billing", fontWeight = FontWeight.Bold, fontSize = 18.sp)
        Text("• Ride Base: 50 ETB\n• Night Fee: 200 ETB (8PM-6AM)\n• Bajaj Hr: 350 ETB/hr\n• Car Hr: 500 ETB/hr\n• Distance limit: 12km/hr", fontSize = 14.sp)
        Spacer(Modifier.height(20.dp))
        Text("Arba Minch Pride", fontWeight = FontWeight.Bold, fontSize = 18.sp)
        Text("Dedicated to modernizing transport in Southern Ethiopia. Wisdom and Peace in every journey.", fontSize = 14.sp)
    }
}
    ...
    Column(Modifier.fillMaxSize().padding(16.dp)) {
        ...
        Card(Modifier.fillMaxWidth()) {
            Row(Modifier.padding(16.dp), verticalAlignment = Alignment.CenterVertically) {
                Icon(Icons.Filled.NotificationsActive, null, tint = IMPERIAL_BLUE)
                Spacer(Modifier.width(12.dp))
                Column { Text("System Active", fontWeight = FontWeight.Bold); Text("Welcome to the fleet.", fontSize = 12.sp, color = Color.Gray) }
            }
        }
    }
}
...
fun BookingHub(name: String, email: String, prefs: android.content.SharedPreferences) {
    ...
    val prestigeTiles = object : OnlineTileSourceBase("Prestige", 0, 20, 256, ".png", arrayOf("https://mt1.google.com/vt/lyrs=m&")) { override fun getTileURLString(p: Long): String = "$baseUrl&x=${MapTileIndex.getX(p)}&y=${MapTileIndex.getY(p)}&z=${MapTileIndex.getZoom(p)}" }

    LaunchedEffect(activeId) { if(activeId.isNotEmpty()) { FirebaseDatabase.getInstance(DB_URL).getReference("rides/$activeId/status").addValueEventListener(object : ValueEventListener { override fun onDataChange(s: DataSnapshot) { status = s.value?.toString() ?: "IDLE" }; override fun onCancelled(e: DatabaseError) {} }) } }
        ...
        if (status != "IDLE") {
            ...
            Column(modifier = Modifier.align(Alignment.BottomCenter).fillMaxWidth().background(Color.White, RoundedCornerShape(topStart = 24.dp)).padding(24.dp)) {
                ...
                if (step == "PICKUP") Button(onClick = { pickupPt = mapRef?.mapCenter as GeoPoint; step = if(selectedTier.label.contains("Hr")) "CONFIRM" else "DEST" }, modifier = Modifier.fillMaxWidth().height(60.dp)) { Text("SET PICKUP", fontWeight = FontWeight.Bold) }
                    ...
                    Button(onClick = { val id = "R_${System.currentTimeMillis()}"; val pt = mapRef?.mapCenter as GeoPoint; FirebaseDatabase.getInstance(DB_URL).getReference("rides/$id").setValue(mapOf("id" to id, "pName" to name, "status" to "REQUESTED", "price" to "450", "pLat" to pickupPt?.latitude, "pLon" to pickupPt?.longitude, "dLat" to destPt?.latitude, "dLon" to destPt?.longitude, "tier" to selectedTier.label, "pay" to "CASH")); activeId = id; prefs.edit().putString("active_id", id).apply() }, Modifier.fillMaxWidth().height(60.dp), colors = ButtonDefaults.buttonColors(containerColor = IMPERIAL_BLUE)) { Text("BOOK ${selectedTier.label.uppercase()}", fontWeight = FontWeight.Bold) }
...
fun VerificationView(phone: String, prefs: SharedPreferences, onVerify: (String) -> Unit) {
    ...
    Column(Modifier.fillMaxSize().background(Color.White).padding(32.dp), Arrangement.Center, Alignment.CenterHorizontally) {
        ...
        Button(onClick = { onVerify(code) }, Modifier.fillMaxWidth().height(60.dp).padding(top = 20.dp), shape = RoundedCornerShape(16.dp)) { Text("VALIDATE ACCESS", fontWeight = FontWeight.Bold) }
    }
}
