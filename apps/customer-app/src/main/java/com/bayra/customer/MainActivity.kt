...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Configuration.getInstance().userAgentValue = "BayraPrestige_v219"
...
fun PassengerSuperApp() {
    ...
    val prefs = remember { ctx.getSharedPreferences("bayra_p_v219", Context.MODE_PRIVATE) }
                ...
                ModalNavigationDrawer(
                    ...
                    Scaffold(
                        ...
                        Box(modifier = Modifier.padding(padding)) {
                            ...
                            else if (currentView == "ORDERS") HistoryPage(pName)
...
fun BookingHub(
    ...
    val greenHandIcon = remember {
        ...
        canvas.drawCircle(size/2f, size/4f + 10, 30f, paint)
        paint.color = android.graphics.Color.WHITE
        canvas.drawCircle(size/2f, size/4f + 10, 10f, paint)
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        AndroidView(factory = { c -> 
            ...
            pickupPt?.let { Marker(view).apply { position = it; icon = greenHandIcon; setAnchor(Marker.ANCHOR_CENTER, Marker.ANCHOR_BOTTOM) }.also { view.overlays.add(it) } }
            destPt?.let { Marker(view).apply { position = it; icon = greenHandIcon; setAnchor(Marker.ANCHOR_CENTER, Marker.ANCHOR_BOTTOM) }.also { view.overlays.add(it) } }
            view.invalidate()
        })
            ...
            Surface(modifier = Modifier.fillMaxSize(), color = Color.White.copy(alpha = 0.98f)) { 
                Column(verticalArrangement = Arrangement.Center, horizontalAlignment = Alignment.CenterHorizontally, modifier = Modifier.padding(24.dp)) { 
                    if (status == "ARRIVED_DEST") {
                        Text(text = "መድረሻዎ ደርሰዋል", fontSize = 24.sp, fontWeight = FontWeight.Black, color = Color(0xFF2E7D32))
                        Text(text = "ARRIVED AT DESTINATION", fontSize = 12.sp, color = Color.Gray)
                        ...
                        }, modifier = Modifier.fillMaxWidth().height(60.dp), colors = ButtonDefaults.buttonColors(containerColor = IMPERIAL_BLUE)) {
...
fun HistoryPage(name: String) {
    ...
    Column(Modifier.fillMaxSize().padding(16.dp)) { Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) { Text("History", fontSize = 22.sp, fontWeight = FontWeight.Bold); IconButton(onClick = { trips.forEach { it.ref.removeValue() } }) { Icon(Icons.Filled.Delete, null, tint = IMPERIAL_RED) } }; LazyColumn { items(trips) { t -> Card(Modifier.fillMaxWidth().padding(top = 8.dp)) { Row(Modifier.padding(16.dp), horizontalArrangement = Arrangement.SpaceBetween) { Text(t.child("tier").value.toString()); Text("${t.child("price").value} ETB", fontWeight = FontWeight.Bold) } } } } }
}
...
fun LoginView(name: String, phone: String, email: String, onLogin: (String, String, String) -> Unit) {
    ...
    Column(modifier = Modifier.fillMaxSize().background(Color.White).verticalScroll(rememberScrollState()).padding(32.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Center) {
        ...
        Text("BAYRA TRAVEL", fontSize = 28.sp, fontWeight = FontWeight.Black, color = IMPERIAL_BLUE); Text("Welcome to Arba Minch", fontSize = 14.sp, color = Color.Gray)
        Spacer(Modifier.height(32.dp)); OutlinedTextField(value = n, onValueChange = { n = it }, label = { Text("Full Name") }, modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(12.dp))
...
fun VerificationView(phone: String, prefs: SharedPreferences, onVerify: (String) -> Unit) {
    ...
    Column(Modifier.fillMaxSize().background(Color.White).padding(32.dp), Arrangement.Center, Alignment.CenterHorizontally) {
        Image(painterResource(id = R.drawable.logo_passenger), null, Modifier.size(120.dp)); Text("VERIFICATION", fontSize = 24.sp, fontWeight = FontWeight.Black, color = IMPERIAL_BLUE)
