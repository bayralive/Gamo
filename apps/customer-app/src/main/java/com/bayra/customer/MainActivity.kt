                    ...
                    drawerContent = {
                        ModalDrawerSheet {
                            Column(Modifier.padding(20.dp)) {
                                Icon(Icons.Filled.AccountCircle, null, modifier = Modifier.size(64.dp), IMPERIAL_BLUE)
...
fun BookingHub(
    ...
    // 🍭 THE LOLLIPOP MARKER FOR SAVED POINTS
    ...
    Box(Modifier.fillMaxSize()) {
        AndroidView(factory = { c -> 
            MapView(c).apply { 
                ...
                overlays.add(MyLocationNewOverlay(GpsMyLocationProvider(c), this).apply { enableMyLocation() })
                mapRef = this 
            } 
        }, update = { view ->
        ...
        if (status != "IDLE") {
            ...
            // 🔥 FIXED BLACK WATER DROP SELECTOR (Compose Native Path)
            ...
            Column(modifier = Modifier.align(Alignment.BottomCenter).fillMaxWidth().background(Color.White, RoundedCornerShape(topStart = 24.dp)).padding(24.dp)) {
                ...
                if (step == "PICKUP") Button(onClick = { onPointChange(mapRef?.mapCenter as GeoPoint, destPt, if(selectedTier.isHr) "CONFIRM" else "DEST", selectedTier, hrCount) }, modifier = Modifier.fillMaxWidth().height(60.dp)) { Text("SET PICKUP", fontWeight = FontWeight.Bold) }
                    ...
                    }, Modifier.fillMaxWidth().height(65.dp), shape = RoundedCornerShape(16.dp)) { Text("BOOK NOW", fontWeight = FontWeight.ExtraBold) }
                }
            }
        }
    }
...
fun HistoryPage(name: String) {
    ...
    Column(Modifier.fillMaxSize().padding(16.dp)) { Row(Modifier.fillMaxWidth(), Arrangement.SpaceBetween, Alignment.CenterVertically) { Text("History", fontSize = 24.sp, fontWeight = FontWeight.Bold); IconButton(onClick = { trips.forEach { it.ref.removeValue() } }) { Icon(Icons.Filled.Delete, null, tint = IMPERIAL_RED) } }; LazyColumn { items(trips) { t -> Card(Modifier.fillMaxWidth().padding(top = 8.dp)) { Row(modifier = Modifier.padding(16.dp), horizontalArrangement = Arrangement.SpaceBetween) { Text(t.child("tier").value.toString()); Text("${t.child("price").value} ETB", fontWeight = FontWeight.Bold) } } } } }
}
