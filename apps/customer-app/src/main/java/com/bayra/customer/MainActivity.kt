...
fun PassengerSuperApp() {
    ...
    MaterialTheme(colorScheme = lightColorScheme(primary = IMPERIAL_BLUE, secondary = IMPERIAL_RED)) {
        if (!isAuth) {
            if (!isVerifying) {
                LoginView(pName, pPhone) { n, p -> 
                    pName = n; pPhone = p; isVerifying = true 
                    FirebaseDatabase.getInstance(DB_URL).getReference("verifications").child(p).setValue(mapOf("name" to n, "status" to "WAITING"))
                    ctx.startActivity(Intent(Intent.ACTION_VIEW, Uri.parse("https://t.me/+JkCzfLAeEexjZjFk")))
                }
            } else {
