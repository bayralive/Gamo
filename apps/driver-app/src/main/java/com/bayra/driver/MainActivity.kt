...
fun DriverAppRoot() {
    ...
    val prefs = remember { ctx.getSharedPreferences("bayra_d_v178", Context.MODE_PRIVATE) }
        ...
        Scaffold(
            bottomBar = {
                NavigationBar(containerColor = Color.Black) {
                    NavigationBarItem(
                        ...
                        colors = NavigationBarItemDefaults.colors(selectedIconColor = Color.Green, unselectedIconColor = Color.Gray, indicatorColor = Color.DarkGray)
                    )
                    NavigationBarItem(
            ...
            Box(Modifier.padding(padding)) {
                if (currentTab == "HOME") {
                    ...
                    DriverAccountView(dName) { prefs.edit().clear().apply(); isAuth = false }
                }
            }
        }
    }
...
fun DriverAccountView(driverName: String, onLogout: () -> Unit) {
    ...
    Column(Modifier.fillMaxSize().background(Color(0xFFFAFAFA)).padding(24.dp), horizontalAlignment = Alignment.CenterHorizontally) {
        ...
        Button(onClick = { /* History Logic */ }, Modifier.fillMaxWidth().height(55.dp), colors = ButtonDefaults.buttonColors(containerColor = Color(0xFF1A237E))) {
...
// ... (RadarHub Logic remains exactly as Phase 174. It is robust and tested. Ensure you keep it or copy from previous phase if overwriting fully.)
@OptIn(ExperimentalMaterial3Api::class)
@Composable
    ...
    Box(Modifier.fillMaxSize()) {
        ...
        Column(Modifier.align(Alignment.BottomCenter).padding(16.dp)) {
            ...
            if(!isRadarOn) {
               ...
               Button(onClick = { isRadarOn = false }, Modifier.fillMaxWidth().height(50.dp), colors = ButtonDefaults.buttonColors(containerColor = Color.Red)) { Text("GO OFFLINE") }
