                    ...
                    Column(verticalArrangement = Arrangement.Center, horizontalAlignment = Alignment.CenterHorizontally) {
                        Button(
                            ...
                            Column(horizontalAlignment = Alignment.CenterHorizontally) {
                                Icon(Icons.Default.PlayArrow, null, Modifier.size(50.dp))
                                Text(text = "ACTIVATE")
                            }
                        }
                    }
            ...
            if (activeJob != null) {
                    ...
                    Column(modifier = Modifier.padding(24.dp)) {
                        ...
                        Row(modifier = Modifier.fillMaxWidth().padding(top = 16.dp), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            ...
                            Button(onClick = { 
                                ...
                                ref.child(activeJob!!.id).updateChildren(mapOf("status" to next))
                            }, modifier = Modifier.weight(1f)) {
                                Text(when(activeJob!!.status) { "ACCEPTED" -> "ARRIVED"; "ARRIVED" -> "START"; else -> "FINISH" })
                            }
                        }
                    }
                }
                ...
                Surface(modifier = Modifier.fillMaxWidth().padding(bottom = 8.dp), shape = RoundedCornerShape(12.dp), color = Color.Black.copy(alpha=0.8f)) {
                    Row(modifier = Modifier.padding(12.dp), Arrangement.SpaceBetween, Alignment.CenterVertically) {
                        Text(text = if(isRadarOn) "RADAR ONLINE" else "OFFLINE", color = if(isRadarOn) Color.Green else Color.Red, fontWeight = FontWeight.Bold)
                        Text(text = "${availableJobs.size} REQUESTS", color = Color.White, fontSize = 12.sp)
